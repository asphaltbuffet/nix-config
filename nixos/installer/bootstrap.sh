...
setup_repo() {
  ...
  if curl -sf --max-time 5 https://github.com >/dev/null 2>&1; then
    ...
    echo "Files will be saved to /root/bootstrap-${HOSTNAME}/ for manual transfer."
    REPO_WRITABLE=false
  fi
}
...
write_host_files() {
  ...
  local artifact_dir="/root/bootstrap-${HOSTNAME}"
...
build_instructions() {
  ...
  local scp_block=""
  if [[ "$REPO_WRITABLE" == "true" ]]; then
    scp_block="  # Pull the new host files from this live machine
  ...
  scp nixos@${live_ip}:/home/nixos/nix-config/nixos/hosts/${HOSTNAME}/configuration.nix \\
    ...
    scp_block="  # Copy hardware-configuration.nix and configuration.nix.template from USB/transfer
  mkdir -p nixos/hosts/${HOSTNAME}
