...
**Control panel**:
The cabinet's physical inputs, presented to the OS by a Xin-Mo dual-arcade encoder (`16c0:05e1`) as *two* devices — one per player — named `Xin-Mo Xin-Mo Dual Arcade`. Each player has a digital joystick (reported as `ABS_X`/`ABS_Y` with range ±1, so MAME addresses it with `_SWITCH` axis codes rather than analog ones) plus ten buttons: eight action buttons in a 4-over-4 grid, then Select and Start. The rightmost column of the grid is reserved for front-end and UI functions, leaving columns 1–3 of both rows as the conventional six-button fighting layout. The device *name* is load-bearing: RetroArch keys autoconfig profiles by it.
_Avoid_: encoder (that is the board, not the surface), joystick (one part of it), controller (also means the USB console pads)
