...
- ~~attract-mode owns `attract.cfg` after the initial seed, so config changes that alter display sections require an explicit re-seed — deploying alone is not enough.~~ **Superseded by ADR-0013**: `attract.cfg` is Nix-managed; activation alone applies changes.
